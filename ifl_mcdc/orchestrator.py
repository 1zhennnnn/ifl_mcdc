...
class IFLOrchestrator:
    ...
    def __init__(
        ...
        self.sampler = LLMSampler(actual_backend, config.llm_retry_delay)
    ...
    def run(self, source_path: str | Path) -> IFLResult:
        ...
        while matrix.compute_loss() > 0 and iteration < self.config.max_iterations:
            ...
            # Orchestrator 負責領域驗證重試（最多 3 次）
            # LLMSamplingError（JSON 解析失敗）或驗證三次不過 → 跳過本輪，下輪重試
            _current_prompt = p_prompt
            new_case: dict[str, object] | None = None
            for _val_attempt in range(3):
                try:
                    _candidate = self.sampler.sample(_current_prompt)
                except LLMSamplingError:
                    break  # JSON 解析失敗，放棄本輪
                _vr = self.config.domain_validator.validate(
                    json.dumps(_candidate)
                )
                if _vr.passed:
                    new_case = _candidate
                    break
                # 驗證失敗：將 violations 編碼為修正提示，繼續重試
                _current_prompt = self._build_validation_prompt(
                    p_prompt, _vr.to_corrective_prompt()
                )
            if new_case is None:
                loss_history.append(matrix.compute_loss())
                continue

            # 步驟 4：AcceptanceGate 驗證 L(X) 是否下降
            ...
            # SMT 互補模型補完翻轉對另一側：
            # LLM 生成的案例僅覆蓋 gap 的「目標方向」（如 c_i=True），
            # 但 MC/DC unique-cause 需要兩側（c_i=True 與 c_i=False）同時存在於 log，
            # 且其他條件值完全相同。若缺少互補側，flip pair 永遠無法配對成功。
            if smt_result.complement_model:
                ...
                if comp_case:
                    ...
                    if self.gate.evaluate(matrix, log, comp_id):
                        test_suite.append(
                            {**comp_case, "__test_id": comp_id, "__source": "smt"}
                        )
    ...
    def _z3_model_to_python(
        ...
        """將 Z3 model 字典轉換為 Python 原生值（供互補模型補完翻轉對使用）。"""
        import z3 as _z3
    ...
    @staticmethod
    def _build_validation_prompt(original: str, violations: str) -> str:
        """建構領域驗證失敗後的修正提示詞。"""
        return (
